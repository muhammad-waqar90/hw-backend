...
class IuCartCheckoutRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'paymentMethod.id' => [
                'string',
                'required_unless:paymentMethod.save,null',
            ],
            'paymentMethod.save' => [
                'boolean',
                'required_unless:paymentMethod.id,null',
            ],
            // for: [other than stripe]
            'transactionBy' => ['nullable', 'string', Rule::in(array_values(PaymentGatewaysData::getConstants()))],
            'transactionReceipt' => [
                'required_unless:transactionBy,null',
            ],
            // for: [inapp]
            'transactionReceipt.transactionId' => [
                'required_if:transactionBy,' . PaymentGatewaysData::INAPP,
            ],
            'transactionReceipt.transactionReceipt' => [
                'required_if:transactionBy,' . PaymentGatewaysData::INAPP,
            ],
            'different_shipping_address' => [
                'required_if:items.*.type,' . PurchaseItemTypeData::PHYSICAL_PRODUCT,
                'boolean',
            ],
            'shipping_country' => [
                'exclude_if:different_shipping_address,false',
                'required_with:different_shipping_address',
                'string',
                'min:3',
                'max:100',
            ],
